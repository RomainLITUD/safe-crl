"""YAML configuration helpers for cost-free Scaling-CRL environments."""
...
def load_env_config(
    ...
    path = (
        ...
        else Path(config_dir or DEFAULT_CONFIG_DIR) / f"{env_id}.yaml"
    )
    if not path.exists():
    ...
    if config_env_id is not None and config_env_id != env_id:
        raise ValueError(
            f"Config {path} has env_id={config_env_id!r}, expected {env_id!r}."
        )
